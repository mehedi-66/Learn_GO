/*
  ...
  - introduce goroutines which are Go’s version of threads.
  - If you’re new to threads, they are nothing but a sequential control flow in a program.
  - Things get interesting when multiple threads are run in parallel so that a program can make use of multiple CPU cores.
  - Goroutines are initiated using the go keyword. In addition to goroutines,
...
In the example below, we’ll consider 5 goroutines that run in parallel. Let’s suppose we organize a cooking contest between 5 gopher chefs.
